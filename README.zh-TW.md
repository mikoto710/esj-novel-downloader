...
- 📖 **章節範圍下載（Beta）**：詳細資料頁和論壇列表頁可獨立選擇連續章節範圍，匯出 TXT、EPUB 或 HTML。
...
> 此功能屬於 `v1.5.2-beta.1` 公測範圍，穩定版 `v1.5.1` 尚未包含。

1. 在小說詳細資料頁或論壇列表頁按一下獨立的「範圍下載」。
2. 輸入開始與結束章序；範圍從 1 開始且包含頭尾，不支援零散單章。
3. 選取整個目錄時與全本下載等同；真正的範圍任務會保留整本快取，僅匯出選中章節。
4. 範圍下載與同一本書的全本下載互斥；「停止並清除快取」會清除本書全部快取。
5. 匯出檔名會包含範圍，例如 `作品名_第101-120章.epub`。
...
書籍下載任務啟動時會固定本次的插圖設定；任務執行期間在其他頁面修改這項設定，只會影響之後新啟動的任務。
