...
支援下載 **TXT / EPUB / HTML**，並適用於多種頁面類型（小說詳細頁／單章閱讀頁／討論區列表頁）。

## 功能特色

- 📚 **多格式匯出**：全本支援 TXT、EPUB 和 HTML，單章支援 TXT 和 HTML。
- ⚡ **多執行緒擷取與接續下載**：可設定下載執行緒數，並透過 IndexedDB 儲存全本下載進度。
- 🛡️ **完整性檢查與重試**：下載後檢查缺少的章節和異常圖片，並自動嘗試重新擷取異常內容。
...
| :--------------- | :-------------------------- | :------------------- | :----------------------- |
| **小說詳細頁**   | 🟢 全本下載 (TXT/EPUB/HTML) | `scrapers/detail.ts` | `.../detail/123.html`    |
| **討論區列表頁** | 🟢 全本下載 (TXT/EPUB/HTML) | `scrapers/forum.ts`  | `.../forum/123/456/`     |
...
1. 在小說詳細頁或討論區列表頁按一下「全本下載」。
2. 如果有未完成的本機快取，可以接續下載並跳過已完成的章節。
3. 遇到密碼保護章節時，畫面會顯示密碼輸入視窗，可提交密碼解鎖、跳過本章、跳過全部剩餘密碼章節，或取消本次下載。
4. 下載期間可以將進度視窗縮到最小；按一下「取消任務」會停止擷取，並嘗試儲存目前進度。
5. 下載完成後會自動檢查缺少的章節；啟用內文插圖時，也會檢查下載失敗的圖片並嘗試重新擷取。
6. 如果重新擷取後還有章節缺漏，可以選擇：
    - 只重試缺少的章節；
    - 加入明確的缺章說明後繼續匯出；
    - 取消並保留快取。
...
小說詳細頁和討論區列表頁的「全本下載」按鈕旁提供「設定」入口：

- **下載執行緒數**：設定同時發出的章節請求數量，預設為 5。
- **介面語言**：可選擇「自動（跟隨網站）」「簡體中文」或「繁體中文」；手動選擇後會儲存設定，並優先於自動判斷。
- **下載內文插圖**：擷取內文圖片並寫入 EPUB / HTML；會增加下載時間、快取占用空間和檔案大小。
...
下載、快取或匯出發生錯誤時，可以從錯誤視窗或設定中的「診斷紀錄」開啟紀錄，並下載 JSON 或複製摘要。
