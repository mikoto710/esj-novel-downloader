...
- 📖 **章节范围下载（Beta）**: 详情页和论坛列表页可独立选择连续章节范围，导出 TXT、EPUB 或 HTML。
...
> 此功能属于 `v1.5.2-beta.1` 公测范围，稳定版 `v1.5.1` 尚未包含。

1. 在小说详情页或论坛列表页点击独立的“范围下载”。
2. 输入起始和结束章序；范围为 1-based 且包含两端，不支持单个零散章节。
3. 选定整个目录时与全本下载等价；真正的范围任务会保留整本缓存，仅导出选中章节。
4. 范围下载与同书全本下载互斥；“停止并清缓存”会清除本书全部缓存。
5. 导出文件名包含范围，例如 `作品名_第101-120章.epub`。
...
书籍下载任务启动时会固定本次插图设置；任务运行期间在其他页面修改该设置，只影响之后新启动的任务。
